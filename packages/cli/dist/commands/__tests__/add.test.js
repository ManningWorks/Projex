...
describe('add command', () => {
    beforeEach(() => {
        ...
        mockFs.readdir.mockResolvedValue(['ProjectCard.tsx', 'index.ts']);
    ...
    describe('project-card component', () => {
        it('should copy project-card files to correct location', async () => {
            ...
            expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('components/folio/ProjectCard'), { recursive: true });
            expect(mockFs.copyFile).toHaveBeenCalledTimes(2);
        });
        it('should show success message after copy', async () => {
            await add('project-card');
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('project-card added successfully'));
        });
        it('should show import usage', async () => {
            await add('project-card');
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("import { ProjectCard } from './components/folio/ProjectCard'"));
        });
    });
    describe('project-view component', () => {
        it('should copy project-view files to correct location', async () => {
            await add('project-view');
            expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('components/folio/ProjectView'), { recursive: true });
        });
    });
    describe('project-grid component', () => {
        it('should copy project-grid files to correct location', async () => {
            await add('project-grid');
            expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('components/folio/ProjectGrid'), { recursive: true });
        });
    });
    describe('project-list component', () => {
        it('should copy project-list files to correct location', async () => {
            await add('project-list');
            expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('components/folio/ProjectList'), { recursive: true });
        });
    });
    describe('featured-project component', () => {
        it('should copy featured-project files to correct location', async () => {
            ...
            expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('components/folio/FeaturedProject'), { recursive: true });
        });
    });
    describe('invalid component', () => {
        ...
        it('should list available components', async () => {
            ...
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('Available components'));
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('project-card'));
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('project-view'));
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('project-grid'));
        });
        it('should show usage hint', async () => {
            ...
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('Usage: npx folio add'));
        });
    });
    describe('existing files', () => {
        ...
        it('should list existing files when found', async () => {
            ...
            vi.mocked(inquirer.prompt).mockResolvedValueOnce({ overwrite: true });
            await add('project-card');
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('existing file'));
        });
        it('should cancel add if user declines overwrite', async () => {
            mockFs.access.mockResolvedValue(undefined);
            vi.mocked(inquirer.prompt).mockResolvedValueOnce({ overwrite: false });
            await add('project-card');
            expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining('Add cancelled'));
    ...
    describe('file system errors', () => {
        ...
        it('should handle source files not found error', async () => {
            ...
            expect(consoleSpy.error).toHaveBeenCalledWith(expect.stringContaining('Source files not found'));
        });
        it('should handle empty source directory', async () => {
            ...
            expect(consoleSpy.error).toHaveBeenCalledWith(expect.stringContaining('No files found'));
        });
        it('should filter source files to only include .ts and .tsx', async () => {
            mockFs.readdir.mockResolvedValue(['Component.tsx', 'index.ts', 'README.md', 'styles.css']);
            await add('project-card');
            expect(mockFs.copyFile).toHaveBeenCalledTimes(2);
        });
    });
