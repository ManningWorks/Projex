...
import { addTimelineEntry, getProjectIds, ConfigEditorError } from '../lib/config-editor.js'
  ...
  try {
    ...
    const note =
      ...
      (await input({
        ...
        validate: (v: string) => v.trim().length > 0 || 'Note is required',
      }))

    addTimelineEntry(projectId, { date, note }, configPath)

    console.log(
      chalk.green(`✓ Timeline entry added to '${projectId}'`),
    )
  } catch (error) {
